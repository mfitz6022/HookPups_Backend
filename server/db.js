...
module.exports = {
  ...
  getAllConfirmedMatches: (data, callback) => {
    pool.query(`SELECT * FROM dog_matches WHERE there is a corresponding match`, (err, response) => {
      callback(err, response);
    })
  },
  ...
  addAMatch: (data, callback) => {
    pool.query(`INSERT INTO dog_matches (dog_id, match_id) VALUES (${data.dog_id}, ${data.match_id})`, (err, response) => { //use a join statement to get the dog_id by joining on the names with the dog_description table
      callback(err, response);
    })
  },
  ...
  postDogDescription: (data, callback) => {
    pool.query(`INSERT INTO dog_details (owner_name, dog_name, breed, size, personality, description, photos, location) VALUES (${data.owner_name}, ${data.dog_name}, ${data.breed}, ${data.size}, ${data.personality}, ${data.description}, ${data.photos}, ${data.location})`, (err, response) => {
      callback(err, response);
    });
  },
};
