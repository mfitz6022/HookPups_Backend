...
module.exports = {
  ...
  pool.query(`
  SELECT * FROM dog_matches JOIN dog_details AS match_dog ON dog_matches.dog2_id = match_dog.dog_id JOIN dog_details ON dog_matches.dog1_id = dog_details.dog_id WHERE (((dog_details.owner_name = '${data.owner_name}' AND dog_details.dog_name = '${data.dog_name}') OR (match_dog.owner_name = '${data.owner_name}' AND match_dog.dog_name = '${data.dog_name}')) AND accepted = 'false');`, (err, response) => {
    callback(err, response);
  })
  },
